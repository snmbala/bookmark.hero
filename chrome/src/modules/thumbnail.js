...
// URLs that Chrome blocks extensions from capturing (security / platform restriction)
const UNCAPTURABLE_PATTERNS = [
    /^chrome:/i,
    /^chrome-extension:/i,
    /^about:/i,
    /^data:/i,
...
function isUncapturable(url) {
    return UNCAPTURABLE_PATTERNS.some(pattern => pattern.test(url));
}
...
export function captureScreenshot(url, title, updateCallback) {
    ...
    if (isUncapturable(url)) {
        console.info('Thumbmark: screenshot capture is not available for this URL (restricted by Chrome):', url);
