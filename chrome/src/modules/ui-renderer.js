...
import analytics from './analytics-integration.js';
...
export function createBookmarkCard(bookmarkNode, searchTerm, folderName, allBookmarks, filterOptions) {
    ...
    // Track bookmark click/open
    cardThumbnailSection.addEventListener('click', function(e) {
        try {
            const domain = bookmarkNode.url ? new URL(bookmarkNode.url).hostname : 'unknown';
            analytics.trackBookmarkClick(bookmarkNode.id, bookmarkNode.title, domain, 'card');
        } catch (err) {
