...
chrome.bookmarks.onCreated.addListener(function (id, bookmarkInfo) {
	...
	// Chrome blocks captureVisibleTab on restricted URLs — skip silently
	const RESTRICTED_URL = /^(chrome|chrome-extension|about|data|javascript):/i;
	const CHROME_STORE = /^https?:\/\/chromewebstore\.google\.com/i;
	if (RESTRICTED_URL.test(bookmarkInfo.url) || CHROME_STORE.test(bookmarkInfo.url)) {
		return;
	}
